app.controller('trendsController', function ($scope, $controller,trendsService,uploadImgService) {
    ...
    $scope.deleteIds = function () {
        ...
        trendsService.deleteIds($scope.selectIds).success(function (data) {
    ...
    $scope.deleteById = function (id) {
        ...
        trendsService.deleteIds(id).success(function (data) {
    ...
    $scope.updateStatusIds = function (status) {
        ...
        if ($scope.selectIds == null || $scope.selectIds.length == 0) {
            $scope.showWarningInfo('请选择要修改的动态id。');
            return;
        }

        trendsService.updateStatusIds($scope.selectIds, status).success(function (data) {
    ...
    $scope.save = function () {
        ...
        if ($scope.tableEntity.id == null) {
            result = trendsService.add($scope.tableEntity);
        }else{
            result = trendsService.update($scope.tableEntity);
        }
